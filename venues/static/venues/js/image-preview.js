...
function setupSortableSequence(containerId) {
    ...
    const updateProfileFrame = () => {
        ...
        // Remove previous
        if (previousProfile && previousProfile !== firstThumb) {
            previousProfile.classList.remove('profile');
            const oldLabel = previousProfile.querySelector('.profile-label');
            if (oldLabel) oldLabel.remove();
        }

        firstThumb.classList.add('profile');

        if (!firstThumb.querySelector('.profile-label')) {
            const label = document.createElement('span');
            label.className = 'profile-label';
            label.innerText = 'Profile';
            firstThumb.appendChild(label);
        }

        previousProfile = firstThumb;
    };
