...
from venues.views import get_venue_visits_analytics_json
...
class VenueViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def dashboard(self, request, pk=None):
        ...
        reservations = venue.reservations.all().order_by("date", "time")
        upcoming = reservations.filter(date__gte=today, status="pending").order_by("date", "time")
        arrivals = reservations.filter(
            date__gte=today,
            status__in=["accepted", "rejected", "cancelled"],
        ).order_by("date", "time")
        past = reservations.filter(date__lt=today).order_by("-date", "-time")

        ensure_working_days(venue)
        analytics = get_venue_visits_analytics_json(
            request,
            venue_id=venue.id,
        ...
        return Response(
            {
                ...
                "upcoming_reservations": [_reservation_payload(item) for item in upcoming],
                "arrivals": [_reservation_payload(item) for item in arrivals],
                "past_reservations": [_reservation_payload(item) for item in past],
    ...
    def submit_update(self, request, pk=None):
        ...
        return Response(
            {"detail": detail, "venue": VenueSerializer(venue, context={"request": request}).data},
...
#         if kind:
#             if kind == "cafe":
#                 qs = qs.filter(kind__in=["cafe", "bar"])
#             else:
#                 qs = qs.filter(kind=kind)

#         if availability == "available":
#             qs = qs.filter(is_full=False)
#         elif availability == "full":
#             qs = qs.filter(is_full=True)
...
#     @action(detail=True, methods=["get"], url_path="slots")
#     @extend_schema(
#         summary="Get available time slots for a venue",
...
#     @action(detail=True, methods=["post"], url_path="submit-update", permission_classes=[permissions.IsAuthenticated])
#     @extend_schema(
#         request=VenueUpdateRequestSerializer,
#         responses={200: "Venue update request created."},
#         summary="Submit a venue update request",
#     )
#     def submit_update(self, request, pk=None):
#         venue = self.get_object()
#         if not user_can_manage_venue(request.user, venue):
...
#         serializer = VenueUpdateRequestSerializer(data=request.data)
#         serializer.is_valid(raise_exception=True)
#         update_data = serializer.validated_data

#         if getattr(settings, "VENUE_UPDATES_REQUIRE_APPROVAL", True):
#             VenueApplication = None
#             from venues.models import VenueUpdateRequest
#             VenueUpdateRequest.objects.create(
#                 venue=venue,
#                 submitted_by=request.user,
#                 name=update_data.get("name", venue.name),
...
#                 description=update_data.get("description", venue.description),
#             )
#             return Response({"detail": "Venue update request submitted."}, status=status.HTTP_201_CREATED)

#         for field, value in update_data.items():
#             setattr(venue, field, value)
#         venue.save(update_fields=list(update_data.keys()))
#         return Response({"detail": "Venue updated successfully."})

#     @action(detail=True, methods=["post"], url_path="toggle-full", permission_classes=[permissions.IsAuthenticated])
#     @extend_schema(
#         summary="Toggle venue availability status",
...
#         with transaction.atomic():
#             venue = Venue.objects.select_for_update().filter(id=venue.id, owner=request.user).first()
#             if not venue:
...
#         grouping = request.GET.get("group", "daily")
#         analytics_data = get_venue_visits_analytics_json(request, venue_id=venue.id, grouping=grouping)
#         return Response(analytics_data)

#     @action(detail=True, methods=["post"], url_path="reorder-images", permission_classes=[permissions.IsAuthenticated])
#     @extend_schema(
#         summary="Reorder venue images",
#         request={"type": "object", "properties": {"sequence": {"type": "array", "items": {"type": "integer"}}}},
#         responses={200: "Image order updated."},
#     )
#     def reorder_images(self, request, pk=None):
#         venue = self.get_object()
#         if not user_can_manage_venue(request.user, venue):
#             return Response({"detail": "Permission denied."}, status=status.HTTP_403_FORBIDDEN)
#         return _reorder_images(venue, request, VenueImage)

#     @action(detail=True, methods=["post"], url_path="reorder-menu-images", permission_classes=[permissions.IsAuthenticated])
#     @extend_schema(
#         summary="Reorder venue menu images",
#         request={"type": "object", "properties": {"sequence": {"type": "array", "items": {"type": "integer"}}}},
#         responses={200: "Image order updated."},
#     )
#     def reorder_menu_images(self, request, pk=None):
#         venue = self.get_object()
#         if not user_can_manage_venue(request.user, venue):
#             return Response({"detail": "Permission denied."}, status=status.HTTP_403_FORBIDDEN)
#         return _reorder_images(venue, request, VenueMenuImage)

#     @action(detail=True, methods=["get", "post"], url_path="working-hours", permission_classes=[permissions.IsAuthenticated])
#     @extend_schema(
#         summary="Get or update venue working hours",
#         request={"type": "object", "properties": {"working_days": {"type": "array", "items": {"type": "object"}}}},
#         responses={200: "Working hours returned or updated."},
#     )
#     def working_hours(self, request, pk=None):
#         venue = self.get_object()
#         if not user_can_manage_venue(request.user, venue):
...
#                     "open_time": wd.open_time.strftime("%H:%M") if wd.open_time else None,
#                     "close_time": wd.close_time.strftime("%H:%M") if wd.close_time else None,
#                     "closes_next_day": wd.closes_next_day,
#                 }
#                 for wd in qs
#             ]
#             return Response({"working_days": data})
...
#         updated_weekdays = []
#         with transaction.atomic():
#             days_by_weekday = {wd.weekday: wd for wd in qs.select_for_update()}
...
#                     if not open_time or not close_time:
#                         return Response(
#                             {"detail": "open_time and close_time are required when the venue is open."},
#                             status=status.HTTP_400_BAD_REQUEST,
#                         )
#                     try:
#                         day.open_time = datetime.strptime(open_time, "%H:%M").time()
...
#                     except ValueError:
#                         return Response(
#                             {"detail": "open_time and close_time must use HH:MM format."},
#                             status=status.HTTP_400_BAD_REQUEST,
#                         )
#                     day.is_closed = False
#                     if "closes_next_day" in item:
#                         day.closes_next_day = bool(item["closes_next_day"])

#                 day.full_clean()
#                 day.save(update_fields=["is_closed", "open_time", "close_time", "closes_next_day"])
#                 updated_weekdays.append(day.weekday)

#         return Response({"updated_weekdays": updated_weekdays})
