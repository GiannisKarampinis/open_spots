...
function useOrderedImageUploads({ images = [], requiresApproval = true }) {
	...
	// Although the state is written inside this custom hook, React associates it with the component
	// currently executing that hook -ManageVenueTab. A custom hook is not a separate component, 
	// it is just a function that is called during the execution of a component function.
	// When an image operation calls: setImageRecords(...) react updates state and re-renders the component
	// that called this hook, which is ManageVenueTab.
	...
	const activeImageRecords = useMemo(
		...
		() => imageRecords
			.filter((image) => !image.marked_for_deletion) // .filter produces a new array with only the images that are not marked for deletion
			.sort((left, right) => left.order - right.order), // .sort mutates the new array, not the original imageRecords array
		[imageRecords],
	);
	...
	// Building displayable gallery items
	const items = useMemo(() => {
		const previewByToken 	= new Map(newImagePreviews.map((preview) => [preview.token, preview]));
		// creates the final collection consumed by the image-gallery UI.
		// Indexes previews by token:
		// 		new-local-1 -> preview object
		// 		new-local-2 -> preview object
		// This makes preview lookup fast instead of repeatedly searching the array for each new image.
		let 	newIndex = 0;

		return activeImageRecords.map((image, index) => {
			...
			const item = {
				...
				status,
				src: preview.src,
				alt: image.file.name || `New image ${newIndex + 1}`,
			};
			newIndex += 1;
			return item;
		}).filter(Boolean);
	}, [activeImageRecords, newImagePreviews]);
	...
	const pendingDeletedItems = useMemo(() => {
		...
		return pendingDeleted.map((image, index) => ({
			...
			src: mediaUrl(image.url),
			alt: `Image ${activeImageRecords.length + index + 1}`,
		}));
	}, [activeImageRecords.length, imageRecords]);


	const imageOrder = useMemo(() => {
		const newIndexByUid = new Map(newImages.map((image, index) => [image.uid, index]));

		return activeImageRecords.map((image) => {
			if (image.type === "existing") {
